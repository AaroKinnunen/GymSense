...
import fi.digi.savonia.movesense.Models.SaMi.ThingsboardMeasurement;
import fi.digi.savonia.movesense.Models.SaMi.ThinsboardPackage;

public class MeasurementHelper implements Serializable {
 ...
 */
        if(temperatureData.size()>0 || batteryVoltageData.size()>0 || ecgData.size()>0 || linearAccelerationData.size()>0 || gyroscopeData.size()>0 || magnetometerData.size()>0 || heartrateData.size()>0 || imu6Data.size()>0 || imu6mData.size()>0 || imu9Data.size()>0)
        {
            ThinsboardPackage thinsboardPackage = new ThinsboardPackage();
            //thinsboardPackage.SetdeviceName("Testeri");
            //thinsboardPackage.SetdeviceType("AppiMove");
            thinsboardPackage.SetKey(key);
            thinsboardPackage.SetMeasurements(convertToMeasurementData());
            samiMeasurementHelper.SendThings(thinsboardPackage);
            sensorTimestamp=0;
        }
    ...
    private SamiMeasurement[] convertToMeasurementData()
    {
        ...
        while(temperatureData.peek()!=null)
        {
            ...
            samiMeasurements.add(new SamiMeasurement(samiDatas,ConvertMillisToISO8601(temp.second),MeasurementTag,MeasurementObject));
        ...
        if(ecgData.peek()!=null)
        {
            ...
            while(ecgData.peek()!=null)
            {
                ...
                for(int i = 0;i<temp.first.Samples.length;i++)
                {
                    ...
                    samiMeasurements.add(new SamiMeasurement(samiDatas,ConvertMillisToISO8601(SequalTimestamp(temp.second,timeDifferentConstant,i)),MeasurementTag,MeasurementObject));
    ...
    /*
    private ThingsboardMeasurement[] convertToThingsMeasurementData()
    {
        List<ThingsboardMeasurement> samiMeasurements = new ArrayList<>();
        List<SamiData> samiDatas = new ArrayList<>();
        ...
        while(temperatureData.peek()!=null)
        {
            Pair<TemperatureData,Long> temp = temperatureData.poll();
            samiDatas.add(new SamiData(TemperatureDataTag,temp.first.ConvertToCelcius()));
            samiMeasurements.add(new ThingsboardMeasurement(samiDatas,ConvertMillisToISO8601(temp.second)));
            samiDatas.clear();
        ...
        if(ecgData.peek()!=null)
        {
            ...
            while(ecgData.peek()!=null)
            {
                ...
                {
                    samiDatas.add(new SamiData("Average",temp.first.Samples[i]));
                    samiMeasurements.add(new ThingsboardMeasurement(samiDatas,ConvertMillisToISO8601(SequalTimestamp(temp.second,timeDifferentConstant,i))));
                    samiDatas.clear();
                }
        ...
        return samiMeasurements.toArray(new ThingsboardMeasurement[samiMeasurements.size()]);
