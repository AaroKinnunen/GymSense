...
public class MovesenseHelper {
    ...
    MdsSubscription mdsSubscription;
 ...
 */
    ...
    /*
    public LineGraphSeries<DataPoint> getGraphPoints(LineGraphSeries<DataPoint> points){
            ...
            public void onNotification(String s) {
                ConcurrencyHelper.GetInstance().Run(() -> {
                    ...
                    if(sensor == Sensor.ECG){
                        final ECGData ecgModel = new Gson().fromJson((JsonElement) oData, ECGData.class);
                        final int[] ecgSamples = ecgModel.Samples;
    ...
    public boolean isSubscribedOrNot(){
        boolean isSubbed;
        if(hrSubscription != null){
            isSubbed =true;
        }
